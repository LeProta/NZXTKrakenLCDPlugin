...
// Table de tous les Krakens avec LCD
static const std::vector<DeviceInfo> SUPPORTED_DEVICES = {
    //                                                  W    H    HID  stream  fps
...
public:
    ...
    // Liste tous les périphériques compatibles branchés
...
private:
    ...
    // Frame protocol
    bool sendFrameStart(uint32_t jpegSize);
    bool sendFrameEnd();
    ...
    bool winHidRead(uint8_t* data, size_t maxLen, int& bytesRead);
