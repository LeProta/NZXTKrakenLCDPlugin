...
void SystemSensors::buildSensorMap()
{
    ...
    try {
        ...
        for (const auto& hw : hsMap) {
            for (const auto& s : hw.second) {
                ...
                qInfo().noquote()
                    << "[Sensors] LHM" << QString::fromStdString(hw.first)
        ...
        qWarning() << "[Sensors] LHWM::GetHardwareSensorMap threw an exception "
                      "(lhwm-wrapper.dll present but .NET/driver failing?).";
        return;
    }

    if (all.empty()) {
        qWarning() << "[Sensors] LHWM: empty sensor map. DLL present but no sensors "
    ...
    auto logId = [&](const char* lbl, SensorType t) {
        ...
        qInfo().noquote() << "[Sensors] resolved" << lbl << "->"
