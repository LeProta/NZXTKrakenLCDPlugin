...
bool KrakenLCDWidget::validateAndImport(const QString& path, QPushButton* triggerBtn)
{
    ...
    if (!fi.exists() || !fi.isFile()) {
        qWarning() << "[UI] File not found:" << path;
    ...
    if (!ALLOWED_EXTENSIONS.contains(ext)) {
        qWarning() << "[UI] Extension not allowed:" << ext << "—" << path;
        flashErrorBorder(triggerBtn);
        return false;
    }

    if (fi.size() > MAX_FILE_SIZE) {
        qWarning() << "[UI] File too large:" << fi.size()
    ...
    if (imported.isEmpty()) {
        qWarning() << "[UI] Import failed for" << path;
        flashErrorBorder(triggerBtn);
        return false;
    }
...
void KrakenLCDWidget::saveConfig()
{
    ...
    KrakenUiPersistence::save(m_config);
}
...
void KrakenLCDWidget::buildModePanel_NowPlaying()
{
    ...
    auto* bBg = makeColorButton(m_config.bgColor);
    connectColorButton(bBg, m_config.bgColor);
    bgRow->addWidget(bBg); bgRow->addStretch();
    l->addLayout(bgRow);
