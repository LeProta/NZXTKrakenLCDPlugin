...
static QString mediaBtnStyle(bool dark)
{
    ...
    if (dark)
        return QStringLiteral(
            ...
            "border:1px solid #2A2A2E;border-radius:5px;padding:5px 10px;font-size:11px;}"
            "QPushButton:hover{background:#22222A;}");
    else
        return QStringLiteral(
            "QPushButton{background:#EBEBEE;color:#1A1A1E;"
            "border:1px solid #D0D0D4;border-radius:5px;padding:5px 10px;font-size:11px;}"
            "QPushButton:hover{background:#DDDDE0;}");
}
...
void LCDPreviewWidget::setBrightness(int percent)
{
    percent = std::clamp(percent, 0, 100);
    if (percent == m_brightness) return;
...
void LCDPreviewWidget::paintEvent(QPaintEvent*)
{
    ...
    if (!m_frame.isNull()) {
        ...
        if (m_brightness < 100) {
            const int a = int((1.f - m_brightness / 100.f) * 255.f);
            p.fillRect(6, 6, 208, 208, QColor(0, 0, 0, a));   // clip circulaire déjà actif
        }
    }
    else {
...
void KrakenLCDWidget::flashErrorBorder(QPushButton* btn)
{
    ...
    auto applyBg = [btn, border, fg](const QColor& c) {
        btn->setStyleSheet(
    ...
    QTimer::singleShot(1000, this, [this, btn, errBg, normalBg, applyBg]() {
        if (!btn) return;
        ...
        connect(fade, &QTimer::timeout, this, [this, btn, fade, step, errBg, normalBg, applyBg]() {
            (*step)++;
            ...
            if (*step >= 10) {
                ...
                fade->deleteLater();
                delete step;
                btn->setStyleSheet(mediaBtnStyle(m_darkTheme));
            }
        });
        fade->start();
    });
...
KrakenLCDWidget::KrakenLCDWidget(QWidget* parent) : QWidget(parent)
{
    ...
    if (m_preview) m_preview->setBrightness(savedBrightness);
...
void KrakenLCDWidget::buildMediaContainer()
{
    ...
    m_btnUpload->setStyleSheet(mediaBtnStyle(m_darkTheme));
    m_btnBrowse = new QPushButton("Browse");
    m_btnBrowse->setStyleSheet(mediaBtnStyle(m_darkTheme));
    m_btnRemove = new QPushButton("Remove");
    m_btnRemove->setStyleSheet(mediaBtnStyle(m_darkTheme));
...
void KrakenLCDWidget::buildUI()
{
    ...
    connect(m_brightnessSlider, &QSlider::valueChanged, [this](int v){
        ...
        saveConfig();
        if (m_preview) m_preview->setBrightness(v);   // retour visuel (aperçu seul)
        if (m_worker)
            QMetaObject::invokeMethod(m_worker, "setBrightness", Qt::QueuedConnection, Q_ARG(int, v));
    });
...
void KrakenLCDWidget::onRotateClicked()
{
    ...
    saveConfig();
    if (m_worker)
        QMetaObject::invokeMethod(m_worker, "setRotation", Qt::QueuedConnection,
                                  Q_ARG(int, m_config.rotation));
}

void KrakenLCDWidget::setDarkTheme(bool dark)
{
    m_darkTheme = dark;
    if (m_btnUpload)      m_btnUpload->setStyleSheet(mediaBtnStyle(dark));
    if (m_btnBrowse)      m_btnBrowse->setStyleSheet(mediaBtnStyle(dark));
    if (m_btnRemove)      m_btnRemove->setStyleSheet(mediaBtnStyle(dark));
