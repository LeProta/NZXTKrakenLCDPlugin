...
FrameConfig KrakenUiPersistence::load()
{
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        qWarning() << "[Persistence] Cannot open" << path;
        return cfg;
    }
    ...
    if (err.error != QJsonParseError::NoError) {
        qWarning() << "[Persistence] Invalid JSON:" << err.errorString();
        return cfg;
    }
    ...
    qDebug() << "[Persistence] Config loaded from" << path
...
bool KrakenUiPersistence::save(const FrameConfig& cfg)
{
    ...
    if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        qWarning() << "[Persistence] Cannot write" << path;
        return false;
    }
